...
def run_soundspaces_per_n(n, tmp_scene_dataset, indirect_ray_count=5000, mic_location=None, audio_samples=-1, save_wav=False):
    ...
    audio_sensor_spec.acousticsConfig.globalVolume = 3.0 #adjusted_mp3d:3.0#270.0 # default: 0.25; adjusts the amplitude of the waveform
    ...
    audio_sensor.setAudioMaterialsJSON("data/TRsim_material_config_upto4khz.json")
...
for MIC_IDX in range(MIC_CHANNELS):
    ...
    np.save(f"data/output/TRsim/{FOLDER}/deconvolved_{N}_{INDIRECT_RAY_COUNT}_mic{MIC_IDX}.npy", deconvolved)
    print(f"Saved data/output/TRsim/{FOLDER}/deconvolved_{N}_{INDIRECT_RAY_COUNT}_mic{MIC_IDX}.npy with shape:", deconvolved.shape)