...
def modify_config_jsons(filename, stage_template, tmp_stage, scene_dataset_template, tmp_scene_dataset):
    with open(stage_template, "r") as f:
        stage_cfg = json.load(f)

        stage_cfg["semantic_asset"] = "../../stages/" + filename #f"TRsim_room{R}_{num_removed}_{boxes_str}_semantic.ply"
    ...
    scene_dataset_cfg["semantic_scene_descriptor_instances"]["semantics_mapping"] = f"stages/base/TRsim.house"
...
def run_soundspaces_per_scene(tmp_scene_dataset, indirect_ray_count=5000, mic_location=None, audio_samples=-1, save_wav=False):
...
N = 2**B                # amount of RIRs (up to 1000)            
